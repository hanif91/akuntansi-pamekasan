unit ujpbik;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics,
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, RzTabs, Vcl.ExtCtrls, RzPanel,
  dxSkinsCore, dxSkinOffice2007Black, dxSkinOffice2007Blue,
  dxSkinOffice2010Black, dxSkinOffice2010Blue, dxSkinOffice2010Silver,
  dxSkinOffice2013DarkGray, dxSkinOffice2013LightGray, dxSkinOffice2013White,
  dxSkinOffice2016Colorful, dxSkinOffice2016Dark, dxSkinTheBezier, cxPC,
  cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters, cxStyles,
  cxCustomData, cxFilter, cxData, cxDataStorage, cxEdit, cxNavigator,
  cxDataControllerConditionalFormattingRulesManagerDialog, Data.DB, cxDBData,
  Vcl.Menus, cxContainer, Vcl.ComCtrls, dxCore, cxDateUtils, MemDS, DBAccess,
  MyAccess, cxProgressBar, cxDropDownEdit, RzButton, cxTextEdit, cxCheckBox,
  cxLabel, cxMaskEdit, cxCalendar, Vcl.StdCtrls, cxButtons, cxGridLevel,
  cxClasses, cxGridCustomView, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxGrid, dxDockPanel, dxDockControl, RzLabel, frxClass,
  frxDesgn, frxDBSet, dxSkinsDefaultPainters, scDevExStyleAdapter;

type
  Tfojpbik = class(TForm)
    RzPanel4: TRzPanel;
    RzPageControl1: TRzPageControl;
    Qjra: TMyQuery;
    TabSheet1: TRzTabSheet;
    RzPanel1setorsimp: TRzPanel;
    cxComboBox2: TcxComboBox;
    cxComboBox1: TcxComboBox;
    cxButton1: TcxButton;
    cxLabel1: TcxLabel;
    RzLabel1: TRzLabel;
    cxProgressBar1: TcxProgressBar;
    cxButton2: TcxButton;
    cxButton3: TcxButton;
    cxLabel2: TcxLabel;
    RzLabel2: TRzLabel;
    frxDesigner1: TfrxDesigner;
    replr: TfrxReport;
    SaveDialog1: TSaveDialog;
    dbjra: TfrxDBDataset;
    Button1: TButton;
    procedure RzToolButton1Click(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure RzPageControl1Close(Sender: TObject; var AllowClose: Boolean);
    procedure cxComboBox1PropertiesChange(Sender: TObject);
    procedure cxButton1Click(Sender: TObject);
    procedure cxButton2Click(Sender: TObject);
    procedure cxButton3Click(Sender: TObject);
    procedure Button1Click(Sender: TObject);
    procedure FormCreate(Sender: TObject);
  private
    { Private declarations }
        lastdate : TDate;
  public
    { Public declarations }
    procedure prosesjurnal;
    procedure viewjurnal;
    procedure exportexcel;
    procedure kondisi1;
    procedure kondisi2;
    procedure kondisi3;
    procedure kondisi4;
    procedure kondisiawal;
  end;

var
  fojpbik : Tfojpbik;

implementation

{$R *.dfm}

uses udm,dateutils,strutils;

procedure Tfojpbik.Button1Click(Sender: TObject);
begin
  with dm do
  begin
    Qcek.Close;
    Qcek.SQL.Clear;
    Qcek.SQL.Add('Call gen_jpbik(:periode,:namauser)');
    Qcek.ParamByName('periode').AsString := '1221';
    Qcek.ParamByName('namauser').AsString := userlogin;
    Qcek.Open;

      ShowMessage(Qcek.FieldByName('notrans').AsString);
      Qcek.Next;
      ShowMessage(Qcek.FieldByName('notrans').AsString);
      Qcek.Prior;
      ShowMessage(Qcek.FieldByName('notrans').AsString);
  end;



end;

procedure Tfojpbik.cxButton1Click(Sender: TObject);
begin
  if RzLabel2.Caption = 'Sudah Di Proses' then
  begin
    if MessageDlg('Anda Akan Melakukan PROSES ULANG Jurnal Periode '+cxComboBox1.Text+' ..!'+#13+'Tekan "Yes" Jika Yakin Untuk Melanjutkan!',mtConfirmation,[mbyes,mbno],0)=mrno then
    exit;
  end
  else
  begin
    if MessageDlg('Anda Akan Melakukan PROSES Jurnal Periode '+cxComboBox1.Text+' ..!'+#13+'Tekan "Yes" Jika Yakin Untuk Melanjutkan!',mtConfirmation,[mbyes,mbno],0)=mrno then
    exit;
  end;
  if dm.cekstatusaktifperiodeposting(cxComboBox2.Text)=0 then
  begin
    MessageDlg('Periode Posting "'+UpperCase(cxComboBox1.Text)+'" Sudah Terkunci / Belum Dibuat ..!',mtWarning,[mbok],0);
    Exit;
  end;
  try
    dm.conngudang.Close;
    dm.conngudang.Username := dm.user3;
    dm.conngudang.Password := dm.pass3;
    dm.conngudang.Server := dm.host3;
    dm.conngudang.Port := StrToInt(dm.port3);
    dm.conngudang.Database := dm.db3;
    dm.conngudang.Connect;
    
    kondisi3;
    prosesjurnal;
    kondisi4;

    dm.conngudang.Close;   
  except
    on e :exception do
    begin
      MessageDlg('Error : !'+e.Message,mtWarning,[mbok],0);
      dm.conngudang.Close;
      kondisi4;
    end;

  end;
end;

procedure Tfojpbik.cxButton2Click(Sender: TObject);
begin
  viewjurnal;
end;

procedure Tfojpbik.cxButton3Click(Sender: TObject);
begin
  exportexcel;
end;

procedure Tfojpbik.cxComboBox1PropertiesChange(Sender: TObject);
begin
  cxComboBox2.ItemIndex := cxComboBox1.ItemIndex;
  with dm do
  begin
    Qcek.Close;
    Qcek.SQL.Clear;
    qcek.SQL.Add('select id from jpbik where periode=:periode limit 1');
    Qcek.ParamByName('periode').AsString := cxComboBox2.Text;
    Qcek.Open;
    if Qcek.RecordCount > 0 then
      kondisi2
    else
      kondisi1;
  end;
end;

procedure Tfojpbik.exportexcel;
begin

end;

procedure Tfojpbik.FormCreate(Sender: TObject);
begin
  Self.Width := cxButton3.Left+cxButton3.Width+50;
  Self.Height := cxButton3.Top+cxButton3.Height+50;
end;

procedure Tfojpbik.FormShow(Sender: TObject);
begin
  dm.isicombo2('periodeposting',cxComboBox1,cxComboBox2);
  dm.isicombo2('periodeposting',cxComboBox1,cxComboBox2);
  cxProgressBar1.Position := 0;
  kondisiawal;
end;

procedure Tfojpbik.kondisi1;
begin
  RzLabel2.Caption := 'Belum Di Proses';
  cxButton2.Enabled := false;
  cxButton3.Enabled := false;
  cxButton1.Enabled := true;
  cxButton1.Caption := 'Proses';
end;

procedure Tfojpbik.kondisi2;
begin
  RzLabel2.Caption := 'Sudah Di Proses';
  cxButton2.Enabled := true;
  cxButton3.Enabled := true;
  cxButton1.Enabled := true;
  cxButton1.Caption := 'Proses Ulang';
end;

procedure Tfojpbik.kondisi3;
begin
  cxProgressBar1.Visible := true;
  cxProgressBar1.BringToFront;
end;

procedure Tfojpbik.kondisi4;
begin
  cxProgressBar1.Visible := false;
  cxProgressBar1.SendToBack;
end;

procedure Tfojpbik.kondisiawal;
begin
  kondisi1;
  kondisi4;
end;

procedure Tfojpbik.prosesjurnal;
var
  periode,periode02,notrans : string;
  i : integer;
  date2,dt : tdate;
  tglakhir,flagins : byte;
begin
  periode :=  cxComboBox2.Text;
  date2 := EncodeDate(StrToInt(LeftStr(cxComboBox2.Text,4)),StrToInt(RightStr(cxComboBox2.Text,2)),1);

  tglakhir := DaysInMonth(date2);
  lastdate := EncodeDate(StrToInt(LeftStr(cxComboBox2.Text,4)),StrToInt(RightStr(cxComboBox2.Text,2)),tglakhir);
  periode02 := FormatDateTime('mmyy',lastdate);
  with dm do
  begin
    
    qcekg.Close;
    Qcekg.SQL.Clear;
    Qcekg.SQL.Add(Qjra.SQLLock.Text);
    Qcekg.ParamByName('periode').AsString := periode02;
    Qcekg.Open;
    if Qcekg.RecordCount>0  then
    begin
      Qex.Close;
      Qex.SQL.Clear;
      Qex.SQL.Add('DELETE FROM `bppbulanan` WHERE namauser=:namauser;');
      Qex.ParamByName('namauser').AsString := userlogin;
      Qex.Execute;


      cxProgressBar1.Position := 0;
      cxProgressBar1.Properties.Max := qcekg.RecordCount;
      for i := 1 to qcekg.RecordCount do
      begin    

        dt :=  EncodeDate(StrToInt(LeftStr(Qcekg.FieldByName('tanggal').AsString,4)),StrToInt(MidStr(Qcekg.FieldByName('tanggal').AsString,6,2)),StrToInt(RightStr(Qcekg.FieldByName('tanggal').AsString,2)));
        Qex.Close;
        Qex.SQL.Clear;
        Qex.SQL.Add('INSERT INTO bppbulanan VALUES(0,:notrans,:kodebarang,:Qty,:dumpqty,:hrgjual,:amount,:deskripsi,:jenis,:keperluan,:kodekeper,:tanggaltrx,NULL,:keterangan,NULL,:namauser)');
        Qex.ParamByName('notrans').AsString := Qcekg.FieldByName('no').AsString;          Qex.ParamByName('kodebarang').AsString := Qcekg.FieldByName('kd_brg').AsString;          Qex.ParamByName('Qty').AsCurrency := Qcekg.FieldByName('jml_mnt').AsCurrency;          Qex.ParamByName('dumpqty').AsCurrency := Qcekg.FieldByName('jml_klr').AsCurrency;         Qex.ParamByName('hrgjual').AsCurrency := Qcekg.FieldByName('hrg').AsCurrency;         Qex.ParamByName('amount').AsCurrency := Qcekg.FieldByName('tot').AsCurrency;         Qex.ParamByName('deskripsi').AsString := Qcekg.FieldByName('nm_brg').AsString;          Qex.ParamByName('jenis').AsString := Qcekg.FieldByName('id_kategori').AsString;         Qex.ParamByName('keperluan').AsString := Qcekg.FieldByName('nama').AsString;        Qex.ParamByName('kodekeper').AsString := Qcekg.FieldByName('kodekeper').AsString;         Qex.ParamByName('tanggaltrx').AsDate := dt;        Qex.ParamByName('keterangan').AsString := Qcekg.FieldByName('utk').AsString;        Qex.ParamByName('namauser').AsString := userlogin;
        Qex.Execute;

        qcekg.Next;
        cxProgressBar1.Position := cxProgressBar1.Position+1;
        Application.ProcessMessages;
      end;
    end
    else
    begin
      MessageDlg('Tidak Ada Data Yang akan di proses pada periode : "'+periode,mtWarning,[mbok],0);
      exit;    
    end;
    

    
    Qcek.Close;
    Qcek.SQL.Clear;
    Qcek.SQL.Add('Call gen_jpbik(:namauser)');
//    Qcek.ParamByName('periode').AsString := periode02;
    Qcek.ParamByName('namauser').AsString := userlogin;
    Qcek.Open;
    if Qcek.RecordCount > 0 then
    begin
      Conn1.StartTransaction;

      Qex.Close;
      Qex.SQL.Clear;
      Qex.SQL.Add('delete from jpbik where periode=:periode');
      Qex.ParamByName('periode').AsString := periode;
      Qex.Execute;


      cxProgressBar1.Position := 0;
      cxProgressBar1.Properties.Max := Qcek.RecordCount;
      for i := 1 to Qcek.RecordCount do
      begin
        notrans := Qcek.FieldByName('notrans').AsString;
        //isi debet
        Qex.Close;
        Qex.SQL.Clear;
        Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,totaldebet,urut,ket2,tanggal)');
        Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:totaldebet,:urut,:ket2,:tanggal)');
        Qex.ParamByName('periode').Value := periode;
        Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
        Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
        Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
        Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
        Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('d_1').AsString;
        Qex.ParamByName('namaakun').Value := Qcek.FieldByName('dn_1').AsString;
        Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
        Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
        Qex.ParamByName('totaldebet').Value := Qcek.FieldByName('j_dn_1').AsCurrency;
        Qex.ParamByName('ket2').AsString := Qcek.FieldByName('d_1').AsString+' - '+Qcek.FieldByName('dn_1').AsString;
        Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
        Qex.ParamByName('urut').AsInteger := 1;
        Qex.Execute;

          //cek flag insert
      {  if i=Qcek.RecordCount then
        begin
          flagins := 1;
        end
        else
        begin
          Qcek.Next;
          if notrans<>Qcek.FieldByName('notrans').AsString then
          begin
            flagins := 1;
          end
          else
          begin
            flagins := 0;
            jmlurut := jmlurut+1;
          end;
          Qcek.Prior;
        end;
                }
        //insert kredit

        if Qcek.FieldByName('k_1').AsString<>'0' then
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,kimia,urut,ket2,tanggal)');
          Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:kimia,:urut,:ket2,:tanggal)');
          Qex.ParamByName('periode').Value := periode;
          Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
          Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
          Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
          Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
          Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('k_1').AsString;
          Qex.ParamByName('namaakun').Value := Qcek.FieldByName('kn_1').AsString;
          Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
          Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
          Qex.ParamByName('kimia').Value := Qcek.FieldByName('j_kn_1').AsCurrency;
          Qex.ParamByName('ket2').AsString := '        '+Qcek.FieldByName('k_1').AsString+' - '+Qcek.FieldByName('kn_1').AsString;
          Qex.ParamByName('urut').AsInteger := 2;
          Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
          Qex.Execute;
        end;

        //isi kredit adm
        if Qcek.FieldByName('k_2').AsString<>'0' then
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,pipa,urut,ket2,tanggal)');
          Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:pipa,:urut,:ket2,:tanggal)');
          Qex.ParamByName('periode').Value := periode;
          Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
          Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
          Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
          Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
          Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('k_2').AsString;
          Qex.ParamByName('namaakun').Value := Qcek.FieldByName('kn_2').AsString;
          Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
          Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
          Qex.ParamByName('pipa').Value := Qcek.FieldByName('j_kn_2').AsCurrency;
          Qex.ParamByName('ket2').AsString := '        '+Qcek.FieldByName('k_2').AsString+' - '+Qcek.FieldByName('kn_2').AsString;
          Qex.ParamByName('urut').AsInteger := 2;
          Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
          Qex.Execute;
        end;

        //isi kredit pemel
        if Qcek.FieldByName('k_3').AsString<>'0' then
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,wm,urut,ket2,tanggal)');
          Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:wm,:urut,:ket2,:tanggal)');
          Qex.ParamByName('periode').Value := periode;
          Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
          Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
          Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
          Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
          Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('k_3').AsString;
          Qex.ParamByName('namaakun').Value := Qcek.FieldByName('kn_3').AsString;
          Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
          Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
          Qex.ParamByName('wm').Value := Qcek.FieldByName('j_kn_3').AsCurrency;
          Qex.ParamByName('ket2').AsString := '        '+Qcek.FieldByName('k_3').AsString+' - '+Qcek.FieldByName('kn_3').AsString;
          Qex.ParamByName('urut').AsInteger := 2;
          Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
          Qex.Execute;
        end;

        //isi kredit retribus
        if Qcek.FieldByName('k_4').AsString<>'0' then
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,pompa,urut,ket2,tanggal)');
          Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:pompa,:urut,:ket2,:tanggal)');
          Qex.ParamByName('periode').Value := periode;
          Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
          Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
          Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
          Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
          Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('k_4').AsString;
          Qex.ParamByName('namaakun').Value := Qcek.FieldByName('kn_4').AsString;
          Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
          Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
          Qex.ParamByName('pompa').Value := Qcek.FieldByName('j_kn_4').AsCurrency;
          Qex.ParamByName('ket2').AsString := '        '+Qcek.FieldByName('k_4').AsString+' - '+Qcek.FieldByName('kn_4').AsString;
          Qex.ParamByName('urut').AsInteger := 2;
          Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
          Qex.Execute;
        end;

        //isi kredit materai
        if Qcek.FieldByName('k_5').AsString<>'0' then
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,acclain,urut,ket2,tanggal)');
          Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:acclain,:urut,:ket2,:tanggal)');
          Qex.ParamByName('periode').Value := periode;
          Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
          Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
          Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
          Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
          Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('k_5').AsString;
          Qex.ParamByName('namaakun').Value := Qcek.FieldByName('kn_5').AsString;
          Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
          Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
          Qex.ParamByName('acclain').Value := Qcek.FieldByName('j_kn_5').AsCurrency;
          Qex.ParamByName('ket2').AsString := '        '+Qcek.FieldByName('k_5').AsString+' - '+Qcek.FieldByName('kn_5').AsString;
          Qex.ParamByName('urut').AsInteger := 2;
          Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
          Qex.Execute;
        end;

        if Qcek.FieldByName('k_6').AsString<>'0' then
        begin
           Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('INSERT INTO jpbik (periode,kodejenis,kodekep,kodeunit,namaunit,kodeakun,namaakun,noref,keterangan,atk,urut,ket2,tanggal)');
          Qex.SQL.Add('Value (:periode,:kodejenis,:kodekep,:kodeunit,:namaunit,:kodeakun,:namaakun,:noref,:keterangan,:atk,:urut,:ket2,:tanggal)');
          Qex.ParamByName('periode').Value := periode;
          Qex.ParamByName('kodejenis').Value := Qcek.FieldByName('kodejenis').AsString;
          Qex.ParamByName('kodekep').Value := Qcek.FieldByName('kodekeper').AsString;
          Qex.ParamByName('kodeunit').Value := Qcek.FieldByName('kodeunit').AsString;
          Qex.ParamByName('namaunit').Value := Qcek.FieldByName('namaunit').AsString;
          Qex.ParamByName('kodeakun').Value := Qcek.FieldByName('k_6').AsString;
          Qex.ParamByName('namaakun').Value := Qcek.FieldByName('kn_6').AsString;
          Qex.ParamByName('keterangan').Value := Qcek.FieldByName('keterangan').AsString;
          Qex.ParamByName('noref').Value := Qcek.FieldByName('notrans').AsString;
          Qex.ParamByName('atk').Value := Qcek.FieldByName('j_kn_6').AsCurrency;
          Qex.ParamByName('ket2').AsString := '        '+Qcek.FieldByName('k_6').AsString+' - '+Qcek.FieldByName('kn_6').AsString;
          Qex.ParamByName('urut').AsInteger := 2;
          Qex.ParamByName('tanggal').AsDate := Qcek.FieldByName('tanggaltrx').AsDateTime;
          Qex.Execute;
        end;


        Qcek.Next;
        cxProgressBar1.Position := cxProgressBar1.Position+1;
        Application.ProcessMessages;
      end;
      Conn1.Commit;
      viewjurnal;
    end
    else
    begin
      MessageDlg('Tidak Ada Data Yang akan di proses pada periode : "'+periode,mtWarning,[mbok],0);
    end;
  end;
end;

procedure Tfojpbik.RzPageControl1Close(Sender: TObject; var AllowClose: Boolean);
begin
  close;
end;

procedure Tfojpbik.RzToolButton1Click(Sender: TObject);
begin
  if (trim(cxComboBox2.Text)='') or ((trim(cxComboBox1.Text)='')) then
  begin
    MessageDlg('Periode Harus diisi !',mtWarning,[mbok],0);
    cxComboBox1.SetFocus;
    Exit;
  end;
end;

procedure Tfojpbik.viewjurnal;
var
  date2 : tdate;
  tglakhir : byte;
  dirlogo : string;
begin
  Qjra.Close;
  Qjra.SQL.Clear;
  Qjra.SQL.Add(Qjra.SQLRefresh.Text);
  Qjra.ParamByName('periode').AsString := cxComboBox2.Text;
  Qjra.Open;

  date2 := EncodeDate(StrToInt(LeftStr(cxComboBox2.Text,4)),StrToInt(RightStr(cxComboBox2.Text,2)),1);
  tglakhir := DaysInMonth(date2);
  lastdate := EncodeDate(StrToInt(LeftStr(cxComboBox2.Text,4)),StrToInt(RightStr(cxComboBox2.Text,2)),tglakhir);
  if Qjra.RecordCount > 0 then
  begin
    dm.isidatattd('jpbik',userlogin);

    replr.LoadFromFile(ExtractFilePath(Application.ExeName)+'\report\jpbik.fr3');

    dirlogo := ExtractFilePath(Application.ExeName)+'\logo\Kop Laporan.png';

    if FileExists(dirlogo) then
    begin
      TfrxPictureView(replr.FindObject('Picture1')).Picture.LoadFromFile(dirlogo);
      //TfrxMemoView(lapkm.FindObject('header1')).Left := 3;
      //TfrxMemoView(lapkm.FindObject('header2')).Left := 3;
     // TfrxMemoView(lapkm.FindObject('alamat1')).Left := 3;
     // TfrxMemoView(lapkm.FindObject('alamat2')).Left := 3;

    end
    else
    begin
      TfrxPictureView(replr.FindObject('Picture1')).Picture.Graphic := nil;
      TfrxMemoView(replr.FindObject('header1')).Left := 0;
      TfrxMemoView(replr.FindObject('header2')).Left := 0;
      TfrxMemoView(replr.FindObject('alamat1')).Left := 0;
      TfrxMemoView(replr.FindObject('alamat2')).Left := 0;
    end;

    TfrxMemoView(replr.FindObject('header1')).Memo.Text:= headerlap1;
    TfrxMemoView(replr.FindObject('header2')).Memo.Text:= headerlap2;
    TfrxMemoView(replr.FindObject('alamat1')).Memo.Text:= alamat1;
    TfrxMemoView(replr.FindObject('alamat2')).Memo.Text:= alamat2;

    TfrxMemoView(replr.FindObject('periode')).Memo.Text:= 'Periode '+cxComboBox1.Text;
    TfrxMemoView(replr.FindObject('footer2')).Memo.Text:= 'JRA '+cxComboBox1.Text;
    TfrxMemoView(replr.FindObject('tanggalbawah')).Memo.Text:= footerkota+', '+FormatDateTime('dd mmmm yyyy',lastdate);
    replr.ShowReport();
  end
  else
  begin
    MessageDlg('Tidak ada data ..!',mtWarning,[mbok],0);
  end;
end;

end.
