...
type
  Tfotambahkaskeluar = class(TForm)
    RzPanel4: TRzPanel;
...
var
  fotambahkaskeluar: Tfotambahkaskeluar;

implementation

{$R *.dfm}

procedure Tfotambahkaskeluar.btnaddClick(Sender: TObject);
begin
...
procedure Tfotambahkaskeluar.btnkembaliClick(Sender: TObject);
begin
  kondisiawal1;
end;

procedure Tfotambahkaskeluar.btnnextClick(Sender: TObject);
begin
...
procedure Tfotambahkaskeluar.btnsimpanClick(Sender: TObject);
var
  i : byte;
begin
      ...
      begin
        try
          ...
          begin
            Qex.Close;
            Qex.SQL.Clear;
            Qex.SQL.Add('insert into d_kaskeluar (notrans,namaketkas,jumlah) value ((Select noautokk(:tgl)),:namaketkas,:jumlah)');
          ...
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('insert into m_kaskeluar (notrans,tanggal,keterangan,total,kas,waktuinput,userinput) value ((Select noautokk(:tgl)),:tgl,:keterangan,:total,:kas,NOW(),:userinput)');
          ...
          begin
            try
              ...
              begin
                Qex.Close;
                Qex.SQL.Clear;
                Qex.SQL.Add('insert into d_kaskeluar (notrans,namaketkas,jumlah) value (:notrans,:namaketkas,:jumlah)');
                Qex.ParamByName('namaketkas').AsString := dxMemData1ket.Text;
                Qex.ParamByName('jumlah').AsCurrency := dxMemData1jumlah.Value;
                Qex.ParamByName('notrans').AsString := cxTextEdit1.Text;
                Qex.Execute;
              ...
              Qex.Close;
              Qex.SQL.Clear;
              Qex.SQL.Add('update m_kaskeluar set tanggal=:tanggal,keterangan=:keterangan,total=:total,kas=:kas,waktuinput=now(),userinput=:userinput where notrans=:notrans');
              Qex.ParamByName('kas').AsString := cxComboBox1.Text;
...
function Tfotambahkaskeluar.cekisiankosong: Boolean;
begin
...
procedure Tfotambahkaskeluar.cxButton1Click(Sender: TObject);
var
  periode : tdate;
begin
  if mode=0 then
  begin
    ...
    begin
      Qcek.Close;
      Qcek.SQL.Clear;
      Qcek.SQL.Add('select noautokk(:tgl)');
...
procedure Tfotambahkaskeluar.cxCurrencyEdit1KeyDown(Sender: TObject;
...
procedure Tfotambahkaskeluar.cxDateEdit1PropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
begin
  cxButton1.Click;
end;

procedure Tfotambahkaskeluar.cxDBLookupComboBox1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if Key=vk_return then
...
procedure Tfotambahkaskeluar.FormShow(Sender: TObject);
begin
...
procedure Tfotambahkaskeluar.kondisiawal1;
begin
...
procedure Tfotambahkaskeluar.kondisiawal2;
begin
...
procedure Tfotambahkaskeluar.kosongkantext1;
begin
...
procedure Tfotambahkaskeluar.kosongkantext2;
begin
  if mode=0 then
  begin
    if flagkosong2=0 then
...
procedure Tfotambahkaskeluar.RzPageControl1Close(Sender: TObject;
