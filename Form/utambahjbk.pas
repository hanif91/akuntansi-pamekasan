...
var
  fotambahjbk: Tfotambahjbk;

implementation

{$R *.dfm}

procedure Tfotambahjbk.cxComboBox1PropertiesChange(Sender: TObject);
begin
  cxComboBox2.ItemIndex := cxComboBox1.ItemIndex;
  kondisi1;
end;

procedure Tfotambahjbk.FormShow(Sender: TObject);
begin
  kondisi1;
  dm.isicombo2('periodeposting',cxComboBox1,cxComboBox2);
end;
                                                    
procedure Tfotambahjbk.kondisi1;
begin
...
procedure Tfotambahjbk.kondisi2;
begin
...
procedure Tfotambahjbk.RzPageControl1Close(Sender: TObject;
  var AllowClose: Boolean);
begin
  self.Close;
end;

procedure Tfotambahjbk.RzToolButton2Click(Sender: TObject);
var
  i,j,idju : integer;
  periode : string;
begin
  ...
  begin
    try
      ...
      Qcek.Open;
      if Qcek.FieldByName('postingjbk').AsInteger=1 then
      begin
      ...
      Qcekkm.Close;
      Qcekkm.SQL.Clear;
      Qcekkm.SQL.Add('CALL cekkaskeluar(:periode)');
   ...
   { {  Qcekkt.Close;
      ...
      Qcekbyadm.Close;
      Qcekbyadm.SQL.Clear;
      Qcekbyadm.SQL.Add('CALL cekpinjaman(:periode)');
      ...
      Qex.Close;
      Qex.SQL.Clear;
      Qex.SQL.Add('UPDATE statusposting set postingjbk="1";');
      Qex.SQL.Add('TRUNCATE dumpjbk;');
      Qex.SQL.Add('TRUNCATE dumpjbk_;');
      Qex.Execute;

      Conn1.Commit;
      
      
      cxMemo1.Lines.Add('   -> Proses Jurnal Transaksi Kas Keluar..');
      ...
      begin
        Qcek.Close;
        Qcek.SQL.Clear;
        Qcek.SQL.Add('SELECT a.*,b.`idcoa` FROM d_kaskeluar a LEFT JOIN ketkas b ON a.namaketkas=b.namaketkas where a.notrans=:notrans');
        ...
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('Insert into dumpjbk (notrans,noref,penjelasan,jumlah,tanggal,tipejurnal) value (:notrans,:noref,:penjelasan,:jumlah,:tanggal,:tipejurnal)');
          Qex.ParamByName('notrans').AsString := 'KK'+IntToStr(i);
          ...
          Qex.ParamByName('tipejurnal').AsString := 'JURNAL BAYAR KAS';
          Qex.Execute;
          ...
          begin
            Qex.Close;
            Qex.SQL.Clear;
            Qex.SQL.Add('Insert into dumpjbk_ (idcoa,notrans,tanggal,jenis,debet,kredit) value (:idcoa,:notrans,:tanggal,:jenis,:debet,:kredit)');
            Qex.ParamByName('idcoa').AsInteger := Qcek.FieldByName('idcoa').AsInteger;
            Qex.ParamByName('notrans').AsString := 'KK'+IntToStr(i);
            Qex.ParamByName('tanggal').AsDate := Qcekkm.FieldByName('tanggal').AsDateTime;
            Qex.ParamByName('jenis').AsString := '';
            Qex.ParamByName('debet').AsCurrency := Qcek.FieldByName('jumlah').AsCurrency;
          ...
          Qex.ParamByName('kredit').AsCurrency := Qcekkm.FieldByName('total').AsCurrency;
          Qex.Execute;
        end;
     ...
     { cxMemo1.Lines.Add('   -> Proses Jurnal Transaksi Kas Transfer..');
      ...
      cxMemo1.Lines.Add('   -> Proses Jurnal Pinjaman Anggota..');
      cxProgressBar1.Position := 0;
      ...
      begin
        ...
        Qex.SQL.Clear;
        Qex.SQL.Add('Insert into dumpjbk (notrans,noref,penjelasan,jumlah,tanggal,tipejurnal) value (:notrans,:noref,:penjelasan,:jumlah,:tanggal,:tipejurnal)');
        Qex.ParamByName('notrans').AsString := 'PJ'+IntToStr(i);
      ...
      Qex.Close;
      Qex.SQL.Clear;
      Qex.SQL.Add('DELETE FROM jbk where DATE_FORMAT(tanggal,"%Y%m")=:periode;');
      Qex.SQL.Add('DELETE FROM jbk_ where DATE_FORMAT(tanggal,"%Y%m")=:periode;');
      Qex.ParamByName('periode').AsString := periode;
      Qex.Execute;

      Qcek.Close;
      Qcek.SQL.Clear;
      Qcek.SQL.Add('SELECT * FROM dumpjbk order by tanggal,noref');
      Qcek.Open;
      ...
      begin
        ...
        begin
          Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('Insert into jbk (notrans,noref,penjelasan,jumlah,tanggal,tipejurnal) value (noautojbk(:tanggal),:noref,:penjelasan,:jumlah,:tanggal,:tipejurnal)');
          ...
          Qdetail.Close;
          Qdetail.SQL.Clear;
          Qdetail.SQL.Add('Select idju from jbk where noref=:noref');
          ...
          Qdetail.Close;
          Qdetail.SQL.Clear;
          Qdetail.SQL.Add('select * from dumpjbk_ where notrans=:notrans order by id');
      ...
      Qex.Close;
      Qex.SQL.Clear;
      Qex.SQL.Add('UPDATE statusposting set postingjbk="0";');
      Qex.Execute;

      cxMemo1.Lines.Add('');
      cxMemo1.Lines.Add('Selesai..');

      MessageDlg('Proses Tambah & Pembaharuan JBK Berhasil .',mtInformation,[mbok],0);
