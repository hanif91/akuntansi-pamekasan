...
procedure Tfolapdph.cxButton1Click(Sender: TObject);
begin
  Qjra.Close;
  Qjra.SQL.Clear;
...
procedure Tfolapdph.cxButton2Click(Sender: TObject);
begin
  viewjurnal;
end;

procedure Tfolapdph.FormCreate(Sender: TObject);
begin
  Self.Width := cxButton2.Left+cxButton2.Width+50;
  Self.Height := cxButton2.Top+cxButton2.Height+50;
  RzDateTimePicker1.Date := date;
  RzDateTimePicker2.Date := date;
end;

procedure Tfolapdph.FormShow(Sender: TObject);
begin
  RzDateTimePicker1.SetFocus;
end;

procedure Tfolapdph.RzPageControl1Close(Sender: TObject;
  var AllowClose: Boolean);
begin
  close;
end;

procedure Tfolapdph.viewjurnal;
var
    dirlogo : string;
begin
  Qjra.Close;
...
//  Qjra.ParamByName('groupusulan').AsCurrency := cxCurrencyEdit1.Value;
  ...
  if Qjra.RecordCount > 0 then
  begin
    dm.isidatattd('dph',userlogin);
    replr.LoadFromFile(ExtractFilePath(Application.ExeName)+'\report\lapdph.fr3');
    ...
    TfrxMemoView(replr.FindObject('alamat2')).Memo.Text:= '';
