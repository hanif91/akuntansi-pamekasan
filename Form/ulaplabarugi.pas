...
uses
  ...
  scDevExStyleAdapter, cxCheckBox;

type
  Tfolaplabarugi = class(TForm)
    RzPanel4: TRzPanel;
    ...
    Qlretap: TMyQuery;
    dblretap1: TfrxDBDataset;
    Qlretap1: TMyQuery;
    dblretap2: TfrxDBDataset;
    Qlretap2: TMyQuery;
...
procedure Tfolaplabarugi.CombKodeunitPropertiesChange(Sender: TObject);
begin
  Combnamaunit.ItemIndex := CombKodeunit.ItemIndex;
end;

procedure Tfolaplabarugi.CombnamaunitPropertiesChange(Sender: TObject);
begin
  CombKodeunit.ItemIndex := Combnamaunit.ItemIndex;
end;

procedure Tfolaplabarugi.cxButton1Click(Sender: TObject);
var
  date2 : tdate;
  tglakhir : byte;
begin
...
procedure Tfolaplabarugi.cxButton2Click(Sender: TObject);
var
  date2 : tdate;
  tglakhir : byte;
  dirlogo : string;
...
procedure Tfolaplabarugi.cxCheckBox2Click(Sender: TObject);
begin
  if cxCheckBox2.Checked then
  begin
...
procedure Tfolaplabarugi.cxComboBox1PropertiesChange(Sender: TObject);
...
procedure Tfolaplabarugi.cxComboBox1PropertiesCloseUp(Sender: TObject);
var
  date2 : tdate;
  tglakhir : byte;
begin
  ...
  begin
    date2 := EncodeDate(StrToInt(LeftStr(cxComboBox2.Text,4)),StrToInt(RightStr(cxComboBox2.Text,2)),1);
    tglakhir := DaysInMonth(date2);
    cxDateEdit1.Date := EncodeDate(StrToInt(LeftStr(cxComboBox2.Text,4)),StrToInt(RightStr(cxComboBox2.Text,2)),tglakhir);
  end;
end;

procedure Tfolaplabarugi.cxGrid1DBBandedTableView1DataControllerSummaryAfterSummary(
  ASender: TcxDataSummary);
...
procedure Tfolaplabarugi.exportlr;
begin
...
procedure Tfolaplabarugi.F7Execute(Sender: TObject);
begin
...
procedure Tfolaplabarugi.FormCreate(Sender: TObject);
begin
...
procedure Tfolaplabarugi.FormShow(Sender: TObject);
begin
...
procedure Tfolaplabarugi.generatelr;
begin
  try
    ...
    try
      ...
      {  Qex.Close;
        ...
        begin
         Qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('CALL `ex_lr_detail_uk`(:awaltahun,:tglbulanini,:periodeini,:namauser,:uk);');
          ...
          Qex.ParamByName('namauser').AsString := userlogin;
          Qex.ParamByName('uk').AsString := CombKodeunit.Text;
          Qex.Execute;
        ...
        Qlretap1.Close;
        Qlretap1.SQL.Clear;
        Qlretap1.SQL.Add(Qlretap1.SQLRefresh.Text);
        Qlretap1.ParamByName('namauser').AsString := userlogin;
        Qlretap1.Open;

        Qlretap2.Close;
        Qlretap2.SQL.Clear;
        Qlretap2.SQL.Add(Qlretap2.SQLRefresh.Text);
        Qlretap2.ParamByName('namauser').AsString := userlogin;
        Qlretap2.Open;
...
procedure Tfolaplabarugi.getakhirbulan(_periode: string);
var
  jmlbln : integer;
begin
...
procedure Tfolaplabarugi.previewreport;
var
  date2 : tdate;
  tglakhir : byte;
  dirlogo : string;
  ...
  begin
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if cxCheckBox2.Checked then
              TfrxMemoView(replr.FindObject('namaunit')).Memo.Text:= 'UNIT : '+UpperCase(Combnamaunit.Text)
            else
              TfrxMemoView(replr.FindObject('namaunit')).Memo.Text:= 'KONSOLIDASI';
    ...
    begin
      ...
      begin
        ...
        if FileExists(dirlogo) then
        begin
          TfrxPictureView(replrrekap.FindObject('Picture1')).Picture.LoadFromFile(dirlogo);
          //TfrxMemoView(lapkm.FindObject('header1')).Left := 3;
          //TfrxMemoView(lapkm.FindObject('header2')).Left := 3;
         // TfrxMemoView(lapkm.FindObject('alamat1')).Left := 3;
         // TfrxMemoView(lapkm.FindObject('alamat2')).Left := 3;
        ...
        begin
          TfrxPictureView(replrrekap.FindObject('Picture1')).Picture.Graphic := nil;
          TfrxMemoView(replrrekap.FindObject('header1')).Left := 0;
          TfrxMemoView(replrrekap.FindObject('header2')).Left := 0;
          TfrxMemoView(replrrekap.FindObject('alamat1')).Left := 0;
          TfrxMemoView(replrrekap.FindObject('alamat2')).Left := 0;
        end;

        TfrxMemoView(replrrekap.FindObject('header1')).Memo.Text:= headerlap1;
        TfrxMemoView(replrrekap.FindObject('header2')).Memo.Text:= headerlap2;
        TfrxMemoView(replrrekap.FindObject('alamat1')).Memo.Text:= alamat1;
        TfrxMemoView(replrrekap.FindObject('alamat2')).Memo.Text:= alamat2;
        if cxCheckBox2.Checked then
          TfrxMemoView(replrrekap.FindObject('namaunit')).Memo.Text:= 'UNIT : '+UpperCase(Combnamaunit.Text)
        else
          TfrxMemoView(replrrekap.FindObject('namaunit')).Memo.Text:= 'KONSOLIDASI';

        TfrxMemoView(replrrekap.FindObject('periode')).Memo.Text:= 'Per '+FormatDateTime('dd mmmm yyyy',tglsdbulanini);
        TfrxMemoView(replrrekap.FindObject('footer2')).Memo.Text:= 'Laporan Laba Rugi '+FormatDateTime('dd mmmm yyyy',tglsdbulanini);
        TfrxMemoView(replrrekap.FindObject('tanggalbawah')).Memo.Text:= footerkota+', '+FormatDateTime('dd mmmm yyyy',cxDateEdit1.Date);
        replrrekap.ShowReport();
      end
      else
      begin
        MessageDlg('Tidak ada data ..!',mtWarning,[mbok],0);
    ...
    else if  cxComboBox3.ItemIndex=2 then
    begin
      if Qlretap.RecordCount > 0 then
      begin
        dm.isidatattd('lr2',userlogin);

        relretap.LoadFromFile(ExtractFilePath(Application.ExeName)+'\report\lr3.fr3');
        ...
        if cxCheckBox2.Checked then
          TfrxMemoView(relretap.FindObject('namaunit')).Memo.Text:= 'UNIT : '+UpperCase(Combnamaunit.Text)
        else
          TfrxMemoView(relretap.FindObject('namaunit')).Memo.Text:= 'KONSOLIDASI';
...
procedure Tfolaplabarugi.RzPageControl1Close(Sender: TObject;
  var AllowClose: Boolean);
begin
  self.Close;
end;

procedure Tfolaplabarugi.Timer1Timer(Sender: TObject);
begin
 if flag=1 then
  begin
