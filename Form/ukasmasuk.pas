...
var fokasmasuk: Tfokasmasuk;

implementation

{$R *.dfm}

procedure Tfokasmasuk.cxCheckBox1Click(Sender: TObject);
begin
...
procedure Tfokasmasuk.cxCheckBox2Click(Sender: TObject);
begin
...
procedure Tfokasmasuk.cxCheckBox3Click(Sender: TObject);
begin
...
procedure Tfokasmasuk.cxCheckBox4Click(Sender: TObject);
begin
...
procedure Tfokasmasuk.cxCheckBox5Click(Sender: TObject);
begin
...
procedure Tfokasmasuk.cxGrid1DBTableView1FocusedRecordChanged
  (Sender: TcxCustomGridTableView; APrevFocusedRecord, AFocusedRecord
  : TcxCustomGridRecord; ANewItemRecordFocusingChanged: Boolean);
begin
  cxLabel2.Caption := 'Detail Kas Masuk No Transaksi : ' + Qkasnotrans.Text;
  QDetail.Close;
  QDetail.SQL.Clear;
...
procedure Tfokasmasuk.cxTextEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if key=#13 then
  begin
    RzToolButton2.Click;
...
procedure Tfokasmasuk.FormShow(Sender: TObject);
begin
  Qkas.Close;
  Qkas.SQL.Clear;
...
procedure Tfokasmasuk.Hapus1Click(Sender: TObject);
begin
  ...
  if MessageDlg('Yakin Menghapus Transaksi Kas Masuk "'+Qkasnotrans.Value+'" ini ?',mtConfirmation,[mbyes,mbno],0)=mryes then
  begin
    with dm do
    begin
      try
        ...
        qex.Close;
        Qex.SQL.Clear;
        Qex.SQL.Add('delete from m_kasmasuk where notrans=:notrans');
        Qex.ParamByName('notrans').AsString := Qkasnotrans.Value;
        Qex.Execute;

        if Qkaskas.Text='KAS TUNAI' then
        begin
          qex.Close;
          Qex.SQL.Clear;
          Qex.SQL.Add('DELETE FROM setorkas WHERE tanggal=:tanggal;');
          Qex.ParamByName('tanggal').AsDate := Qkastanggal.Value;
          Qex.Execute;
        end;
...
procedure Tfokasmasuk.Koreksi1Click(Sender: TObject);
var i, j: byte;
begin
  if dm.cekstatusaktifperiodeposting(FormatDateTime('yyyymm',Qkastanggal.Value))=0 then
  begin
  ...
  begin
    ...
    kaslama := Qkaskas.Text;

    if fotambahkasmasuk.ShowModal = mrok then
    begin
      Qkas.Refresh;
    end;
  end;
...
procedure Tfokasmasuk.RzToolButton1Click(Sender: TObject);
begin
  fotambahkasmasuk.mode := 0;
  if fotambahkasmasuk.ShowModal = mrok then
  begin
    Qkas.Close;
    Qkas.Open;
...
procedure Tfokasmasuk.RzToolButton2Click(Sender: TObject);
begin
  Qkas.Close;
  Qkas.SQL.Clear;
  Qkas.SQL.Add(Qkas.SQLRefresh.Text);
